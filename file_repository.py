...
class FileRepository:
    def __init__(self, people_file, events_file):
    ...
    def load_data(self):
        try:
            with open(self.people_file, 'r') as file:
                ...
                self.people = [Person(*(line.strip().split(','))) for line in lines]

            with open(self.events_file, 'r') as file:
                lines = file.readlines()
                self.events = [Event(*(line.strip().split(','))) for line in lines]

        except FileNotFoundError:
            pass

    def save_data(self):
        with open(self.people_file, 'w') as file:
            for person in self.people:
                file.write(f"{person.get_person_id()},{person.get_name()},{person.get_address()}\n")

        with open(self.events_file, 'w') as file:
            for event in self.events:
                file.write(f"{event.get_event_id()},{event.get_date()},{event.get_time()},{event.get_description()}\n")
